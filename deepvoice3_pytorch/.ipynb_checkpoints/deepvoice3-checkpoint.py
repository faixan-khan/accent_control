...
class AttentionLayer(nn.Module):
    def __init__(self, conv_channels, embed_dim, dropout=0.1,
        ...
        self.query_projection = Linear(conv_channels, conv_channels)
        if key_projection:
            self.key_projection = Linear(embed_dim, conv_channels)
        ...
        if value_projection:
            self.value_projection = Linear(embed_dim, conv_channels)
        else:
            self.value_projection = None

        self.out_projection = Linear(conv_channels, conv_channels)
        self.dropout = dropout
