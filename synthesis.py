...
def tts(model, text, vid_path, p=0, speaker_id=None, fast=False):
	...
	video = np.asarray(video) / 255.
	video_positions = torch.arange(1, video.shape[0] + 1).unsqueeze(0).long().to(device)
	video = torch.from_numpy(video).float().to(device)
	# Greedy decoding
	with torch.no_grad():
		mel_outputs, linear_outputs, alignments, done, alignments_f = model(
			sequence, vid_positions=video_positions, text_positions=text_positions, speaker_ids=speaker_ids, batch_size=1, faces=video)

	linear_output = linear_outputs[0].cpu().data.numpy()
	spectrogram = audio._denormalize(linear_output)
	alignment = alignments[0].cpu().data.numpy()
	alignment_f = alignments_f[0].cpu().data.numpy()
	...
	return waveform, alignment, spectrogram, mel, alignment_f
...
if __name__ == "__main__":
	...
	checkpoint_path = './checkpoint_step000050000.pth'
	# text_list_file_path = args["<text_list_file>"]
	text_list_file_path = 'syn.txt'
	...
	# preset = args["--preset"]
	preset = './presets/nyanko_ljspeech.json'
	...
	with open(text_list_file_path, "r") as f:
		...
		for idx, line in enumerate(lines):
			txt, vid_path = line.strip().split('|')
			if os.path.isfile(vid_path):
				text = txt
				# text = txt.decode("utf-8")[:-1]
				words = nltk.word_tokenize(text)
				waveform, alignment, _, _, alignment_f = tts(
					model, text, vid_path, p=replace_pronunciation_prob, speaker_id=speaker_id, fast=True)
				...
				dst_alignment_path = join(
					dst_dir, "{}_{}{}_alignment.png".format(idx, checkpoint_name,
															file_name_suffix))
				dst_alignment_path_f = join(
					dst_dir, "{}_{}{}_alignment_v.png".format(idx, checkpoint_name,
															file_name_suffix))
				plot_alignment(alignment.T, dst_alignment_path,
							   info="{}, {}".format(hparams.builder, basename(checkpoint_path)))
				plot_alignment(alignment_f.T, dst_alignment_path_f,
							   info="{}, {}".format(hparams.builder, basename(checkpoint_path)))
